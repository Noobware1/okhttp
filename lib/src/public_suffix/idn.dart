...
///
/// this is stolen from https://github.com/Ephenodrom/Dart-Basic-Utils/blob/master/lib/src/library/IDNAConverter.dart
///
...
class IDNAConverter {
  ...
  static String _urlConvert(String url, bool shouldencode) {
      ...
      _split.forEach((elem) {
        ...
        if (shouldencode) {
          if (regexPunycode.hasMatch(elem) ||
            ...
            _join.add('xn--${String.fromCharCodes(encode(elem))}');
          }
        } else {
    ...
    return _result.length > 1 ? _result.join('/') : _result.elementAt(0);
  }
}
